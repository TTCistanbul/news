...
def main():
    ...
    # 第 1 列：CPI。用的是跟首頁核心指標同一組序列，兩處數字保證一致。
    cpi_vals = [e["value"] for e in cpi_series if "value" in e]
    if cpi_vals:
        cpi_row_name = f"消費者物價指數（CPI，{cpi_month_label}）"
        cpi_row_value = f"{cpi_vals[-1]:.2f}%"
        cpi_pp = (cpi_vals[-1] - cpi_vals[-2]) if len(cpi_vals) >= 2 else None
        cpi_row_delta = _pp(cpi_pp)
        cpi_row_delta_cls = _delta_cls(cpi_pp)
        # 「年增率」這一欄先留空。EVDS 抓回來的這支 CPI 序列到底是「年通膨
        # 率」還是「物價指數點數」目前沒有確認過（首頁核心指標把它當年通膨
        # 率顯示成 134.75%，舊版表 02 卻寫 31.75%，兩者不可能同時對）。
        # 如果它是年通膨率，最新值本身就是年增率，這一欄重複；如果它是指數
        # 點數，才需要在這裡算 YoY。在確認之前寧可顯示 — ，不要放一個看起來
        # 合理但可能完全錯誤的百分比。確認後把下一行換成：
        #     cpi_row_yoy = _pct(_yoy(cpi_vals))
        cpi_row_yoy = "—"
    else:
